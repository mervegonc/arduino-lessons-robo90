...
setup(): Kart ilk açıldığında veya reset tuşuna basıldığında yalnızca bir kez çalışır. Genellikle pinlerin giriş mi (INPUT) çıkış mı (OUTPUT) olduğu (pinMode) ve baud hızları (Serial.begin) burada tanımlanır.

loop(): setup() çalışıp bittikten sonra devreye girer ve kart enerjili olduğu sürece kodları satır satır çalıştırıp en alta geldiğinde tekrar başa dönerek sonsuza kadar çalışmaya devam eder.
